...
//    @Override
    public boolean update(User user_1) {
        conn=DBUtils.getConn();
        String sql="update friend_user set `name`=?,`password`=?,`age`=?,`phone`=? where id=?;";
        try {
            ps=conn.prepareStatement(sql);
            ps.setString(1,user_1.getName());
            ps.setString(2,user_1.getPassword());
            ps.setInt(3,user_1.getAge());
            ps.setString(4,user_1.getPhone());
            ps.setInt(5,user_1.getId());
